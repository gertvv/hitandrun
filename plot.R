...
boundBox <- function(x, d) {
	lb <- c(-sqrt(2/3), -sqrt(2/3)) #FIXME: bounds correct for all n?
	ub <- c(sqrt(2/3), sqrt(2/3))
	c(
		max(sapply(1:length(x), function(i) { min((lb[i] - x[i]) / d[i], (ub[i] - x[i]) / d[i]) })),
		min(sapply(1:length(x), function(i) { max((lb[i] - x[i]) / d[i], (ub[i] - x[i]) / d[i]) }))
	)
}
